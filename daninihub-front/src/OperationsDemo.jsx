...
const content = {
  de: {
    ...
    route: 'Novi Sad → Duisburg', cargo: '24 Paletten · Industrieware', client: 'BalkanCargo (fiktiv)', slot: 'Entladeslot', planned: 'Plan-ETA', current: 'Aktuelle ETA', decision: 'Entscheider', documents: 'Dokumente', complete: 'operativ vollständig',
    timeline: 'Status-Zeitlinie', next: 'Nächsten Schritt anzeigen', reset: 'Simulation neu starten', active: 'Aktueller Vorgang', open: 'Offene Entscheidung', noDecision: 'Keine offene Entscheidung', summary: 'Pilot-Zusammenfassung', cta: 'Pilot besprechen',
    steps: [
  ...
  sr: {
    ...
    route: 'Novi Sad → Duisburg', cargo: '24 palete · industrijska roba', client: 'BalkanCargo (fiktivno)', slot: 'Termin istovara', planned: 'Planirana ETA', current: 'Aktuelna ETA', decision: 'Odgovorna osoba', documents: 'Dokumenti', complete: 'operativno kompletni',
    timeline: 'Vremenska linija statusa', next: 'Prikaži sledeći korak', reset: 'Pokreni simulaciju ponovo', active: 'Aktuelni događaj', open: 'Otvorena odluka', noDecision: 'Nema otvorene odluke', summary: 'Rezime pilota', cta: 'Razgovor o pilotu',
    steps: [
      ...
      ['21:32', 'Istovar obavešten', 'Operativni prijem moguć do 11:15. DaniniHub ne menja pravno obavezujuće termin.'],
      ['21:38', 'Učesnici ažurirani', 'Naručilac obavešten. Vozač je dobio odobrenu informaciju. Sledeća provera: 07:00.']
    ],
...
export default function OperationsDemo({lang}) {
  ...
  return <main className="ops-shell">
    <header className="ops-header"><a href={lang==='sr'?'/sr/primer-pilota':'/de/pilot-beispiel'}>← {t.back}</a><strong>DaniniHub · DEMO</strong></header>
    <section className="ops-page">
      ...
      <div className="ops-overview">
        <article><small>TOUR</small><strong>{t.route}</strong><span>{t.cargo}</span></article>
        <article><small>{t.client}</small><strong>{t.slot}: 10:00</strong><span>{t.planned}: 09:20</span></article>
        <article className={issue?'is-alert':''}><small>{t.current}</small><strong>{eta}</strong><span>{issue?'+ 90 Min. Abweichung':'Im Plan'}</span></article>
        <article><small>{t.decision}</small><strong>Herr Marković</strong><span>{t.documents}: {t.complete}</span></article>
      </div>
      <div className="ops-grid">
        <section className="ops-panel"><div className="panel-head"><h2>{t.timeline}</h2><span>{visible}/{t.steps.length}</span></div><div className="ops-timeline">{t.steps.map(([time,title,text],index)=><article key={time+title} className={index<visible?'shown':''}><span className="time">{time}</span><div><strong>{title}</strong><p>{index<visible?text:'••••••••••••••••••••'}</p></div></article>)}</div></section>
      ...
      {finished&&<section className="ops-result"><p className="kicker">AUSWERTUNG · EVALUACIJA</p><h2>{t.summary}</h2><div>{t.metrics.map(([value,label])=><article key={label}><strong>{value}</strong><span>{label}</span></article>)}</div><p>{t.boundary}</p><a className="btn" href={lang==='sr'?'/sr/?interest=pilot#contact':'/de/?interest=pilot#contact'}>{t.cta} →</a></section>}
    </section>
  </main>
}
