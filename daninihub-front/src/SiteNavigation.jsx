...
const links = {
  de: { start:'Start', services:'Leistungen', capacity:'Lkw / Ladung', dispoLab:'DispoLab', scope:'Leistungsrahmen', continuity:'Continuity Support', drivers:'Fahrerkommunikation', pilot:'Pilot-Check', knowledge:'Praxis & Wissen', contact:'Kontakt', menu:'Menü', close:'Schließen' },
  sr: { start:'Početna', services:'Usluge', capacity:'Kamion / teret', dispoLab:'DispoLab', scope:'Obim usluge', continuity:'Podrška kontinuitetu', drivers:'Komunikacija sa vozačima', pilot:'Provera pilota', knowledge:'Praksa i znanje', contact:'Kontakt', menu:'Meni', close:'Zatvori' }
}
...
export default function SiteNavigation({ lang }) {
  ...
  const scope = sr ? '/sr/obim-usluge' : '/de/leistungsrahmen'
  const serviceLinks = [
    [t.scope, scope],
  ...
  return <>
    <header className="site-nav">
      ...
      <div className="site-nav-actions"><div className="site-nav-langs" aria-label="Language"><a className={lang === 'de' ? 'active' : ''} href={deHref}>DE</a><a className={lang === 'sr' ? 'active' : ''} href={srHref}>SR</a></div><a className="site-nav-cta" href={pilot}>{t.pilot}</a></div>
