...
export default function TransportNetworkDemo({ lang }) {
  ...
  const load = async active => {
    setState('loading')
    try {
      const response = await fetch('/api/v1/transport-network/workspace', { headers: { Authorization: `Bearer ${active || token}` } })
  ...
  const partners = data.company.companyId === 'CMP-DACH-001'
  ...
  return <main className="tn-shell">
    ...
    <section className="tn-overview"><article><small>{t.roomCount}</small><strong>{data.rooms.length}</strong></article><article><small>{t.teamCount}</small><strong>{data.members.length}</strong></article><article><small>{t.status}</small><strong>{data.company.status}</strong></article></section>
    ...
    {view === 'rooms' && <section className="tn-card"><h2>{t.rooms}</h2><p className="tn-help">{t.existingHelp}</p>{state === 'open-error' && <p className="tn-error">{t.openError} {errorCode && <>· {t.errorCode}: {errorCode}</>}</p>}<div className="tn-rooms">{data.rooms.map(room => <article key={room.caseId}><small>{room.caseId}</small><h3>{room.route}</h3><p>{t.status}: <b>{room.status}</b></p><p>{t.eta}: <b>{room.eta}</b> · {t.risk}: <b>{room.risk}</b></p><button onClick={() => openRoom(room.caseId)} disabled={opening === room.caseId}>{opening === room.caseId ? t.opening : t.openRoom} →</button></article>)}</div></section>}

    {view === 'team' && <section className="tn-grid"><article className="tn-card"><h2>{t.companyInfo}</h2><dl><div><dt>ID</dt><dd>{data.company.companyId}</dd></div><div><dt>Type</dt><dd>{data.company.type}</dd></div><div><dt>Status</dt><dd>{data.company.status}</dd></div><div><dt>Documents</dt><dd>{data.company.documentStatus}</dd></div><div><dt>Routes</dt><dd>{data.company.routes.join(' · ')}</dd></div><div><dt>{t.storage}</dt><dd>{data.storageMode}</dd></div></dl></article><article className="tn-card"><h2>{t.members}</h2><p className="tn-help">{t.memberHelp}</p><div className="tn-list">{data.members.map(member => <div key={member.memberId}><strong>{member.name}</strong><span>{member.email}</span><b>{member.role}</b></div>)}</div><details className="tn-admin-details"><summary>{t.addMember}</summary><form onSubmit={addMember}><input name="name" placeholder={t.name} required/><input name="email" type="email" placeholder={t.email} required/><select name="role"><option>DISPATCHER</option><option>VIEWER</option></select><button>{t.create}</button></form></details></article></section>}
