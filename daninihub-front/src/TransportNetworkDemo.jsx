...
export default function TransportNetworkDemo({lang}){
 ...
 return <main className="tn-shell">
  ...
  <section className="tn-card"><h2>{t.rooms}</h2>{state==='open-error'&&<p className="tn-error">{t.openError}</p>}<div className="tn-rooms">{data.rooms.map(r=><article key={r.caseId}><small>{r.caseId}</small><h3>{r.route}</h3><p>{t.status}: <b>{r.status}</b></p><p>{t.eta}: <b>{r.eta}</b> · {t.risk}: <b>{r.risk}</b></p><button onClick={()=>openRoom(r.caseId)} disabled={opening===r.caseId}>{opening===r.caseId?t.opening:t.openRoom} →</button></article>)}</div><form className="tn-room-form" onSubmit={addRoom}><input name="route" placeholder={t.route} required/><select name="partnerCompanyId">{partners.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><button>{t.newRoom}</button></form></section>
 </main>
}
