...
function sanitize(input={}){
  ...
  return {
    ...
    contactPerson: String(input.contactPerson||'').trim().slice(0,180),
    postalAddress: String(input.postalAddress||'').trim().slice(0,300),
    contactChannel: String(input.contactChannel||'').trim().slice(0,80),
    ...
    postalLetter: String(input.postalLetter||'').trim().slice(0,5000),
