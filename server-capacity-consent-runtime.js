...
function renderPage() {
  ...
  *{box-sizing:border-box}body{margin:0;background:#07131f;color:#eaf2f5;font-family:Inter,Arial,sans-serif}.wrap{max-width:1250px;margin:auto;padding:28px 18px 70px}.brand{font-size:13px;font-weight:900;letter-spacing:.12em;color:#67d7e4}h1{font-size:clamp(34px,5vw,54px);margin:10px 0 4px}p{color:#b9c9d2}.auth,.actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center}.auth{margin:22px 0}.auth input,select,textarea,button{font:inherit;border:1px solid #3a5262;border-radius:10px;background:#0d2030;color:#fff;padding:11px 12px}button{background:#0d8b98;border-color:#0d8b98;font-weight:800;cursor:pointer}.secondary{background:#193245}.msg{min-height:24px;color:#ffb4a4}.list{display:grid;gap:16px}.case{border:1px solid #29495b;border-radius:17px;background:#0b1c2a;padding:18px}.head{display:flex;justify-content:space-between;gap:16px;flex-wrap:wrap}.tag{display:inline-block;padding:5px 9px;border-radius:999px;background:#123848;color:#8ee8f2;font-size:12px;font-weight:900}.ref{font-family:ui-monospace,monospace;color:#a8c0cb}.grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:14px}.side{border:1px solid #29495b;border-radius:12px;padding:14px;background:#071722}.side h3{margin:0 0 10px}.side textarea{width:100%;min-height:75px;margin-top:8px}.status{font-size:28px;font-weight:900}.empty{padding:28px;border:1px dashed #3a5262;border-radius:14px;text-align:center}@media(max-width:760px){.grid{grid-template-columns:1fr}.auth input{width:100%}}
  ...
  function render(cases){list.innerHTML=cases.length?cases.map(c=>'<article class="case"><div class="head"><div><span class="tag">'+esc(c.status)+'</span><h2>'+esc(c.truckCompany||c.truckReference)+' ↔ '+esc(c.freightCompany||c.freightReference)+'</h2><div class="ref">'+esc(c.id)+'</div></div><div class="status">'+esc(c.score)+'/100</div></div><p>'+esc(c.truckReference)+' · '+esc(c.freightReference)+'</p><div class="grid">'+side('truck','Firma sa kamionom',c.truckConsent,c.id)+side('freight','Firma sa teretom',c.freightConsent,c.id)+'</div></article>').join(''):'<div class="empty">Nema otvorenih predmeta.</div>'}
  async function load(){msg.textContent='';sessionStorage.setItem('danini_signal_admin_secret',key.value);const d=await request('/api/v1/signal-consents/cases');render(d.cases||[])}
...
function mountCapacityConsentRuntime(app, options = {}) {
  ...
  app.get('/api/v1/signal-consents/cases', requireAdmin, async (req,res) => {
    const cases = Object.values(readCases());
    const leads = await store.list({ sources:['capacity-truck','capacity-freight'], limit:500 });
    const map = new Map(leads.map(lead => [lead.reference, publicSignal(lead)]));
    return res.json({ ok:true, cases:cases.map(record => ({ ...record, truckCompany:map.get(record.truckReference)?.company || '', freightCompany:map.get(record.freightReference)?.company || '' })) });
  });
