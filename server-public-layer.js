function getPublicWebLayerPayload(activeRoute = '/') {
  return {
    ...
    artifacts: [
      ...
      { id: 'digitale-standortvermarktung-dpl', title: 'Digitale Standortvermarktung / Digitalna prodaja lokacije', status: 'existing_product_delivery_validation_required', route: '/artifacts#digitale-standortvermarktung-dpl' },
      { id: 'calije-park-residence', title: 'Calije Park Residence', status: 'public_artifact_private_boundary_required', route: '/artifacts#calije-park-residence' }
    ],
    blockedAutomation: [
...
const LANGUAGE = {
  de: {
    code: 'de', base: '', label: 'DE', dir: '', method: 'Frage KI - KI fragt dich',
    nav: { method: 'Methode', mode: 'Projektmodus', prices: 'Preise', artifacts: 'Artifacts', entry: 'ENTRY' },
    badge: 'USTAV-first - DACH-ready - AI Transparency',
    hero: 'DaniniHub ist kein Chatbot. DaniniHub ist ein strukturierter Projektmodus.',
    lead: 'Ein Decision Engine fuer Menschen, die mit KI nicht nur Antworten sammeln, sondern bessere Fragen stellen, Projekte klaeren und belastbare Artifacts erzeugen wollen.',
    ...
    cards: [
      ...
      ['Project Mode', 'Gate 0-5', 'Ein aktives Projekt wird durch strukturierte Gates gefuehrt, mit Status, Review-Logik und Artifact-Fokus.'],
  ...
  sr: {
    code: 'sr', base: '/sr', label: 'SR', dir: '/sr', method: 'Pitaj AI - AI pita tebe',
    ...
    cards: [
      ...
      ['Projektni mod', 'Gate 0-5', 'Jedan aktivan projekat prolazi kroz strukturisane gate faze, status, review logiku i artifact fokus.'],
  ...
  en: {
    code: 'en', base: '/en', label: 'EN', dir: '/en', method: 'Ask AI - AI asks you',
...
function getPageMeta(activeRoute) {
  ...
  const clean = stripLang(activeRoute);
  return payload.pages.find((page) => page.route === clean) || payload.pages[0];
}
...
function renderPublicHtml(activeRoute = '/') {
  ...
  const page = getPageMeta(cleanRoute);
  ...
  const artifactsHtml = payload.artifacts.map((artifact) => `
    <article class="card" id="${escapeHtml(artifact.id)}">
      ...
      <a href="${escapeHtml(navHref(artifact.route))}">Mehr / Vise / More</a>
    </article>
  `).join('');
...
<head>
  ...
  <meta name="description" content="DaniniHub: USTAV-first Decision Engine, Project Mode and AI-supported artifact system." />
...
<body>
  ...
  <main>
    ...
    <section class="wrap"><div class="panel"><p class="eyebrow">Public Web Layer</p><h2>${escapeHtml(page.title)}</h2><p class="trust">${escapeHtml(page.purpose)}</p></div></section>
    <section class="wrap"><p class="eyebrow">Artifacts</p><div class="grid">${artifactsHtml}</div></section>
    <section class="wrap"><p class="eyebrow">System Navigation</p><div class="grid">${pagesHtml}</div></section>
    <section class="wrap"><div class="panel trust" id="entry-not-configured"><strong>Transparency:</strong> ${escapeHtml(t.transparency)}</div></section>
  </main>
