...
const ROUTES={
  ...
  '/sr/':{lang:'sr',title:'DANINI | Uvoz automobila iz Nemačke i Švajcarske u Srbiju',description:'Pre kupovine proveri troškove, poreklo i rizik. Organizuj pregled auta, originalne delove, dovoz i bazu u Nišu.'},
...
function home(lang){
  const sr=lang==='sr';
  const nav=sr
    ? [['passport','Provera uvoza'],['inspection','Pregled auta'],['parts','Delovi'],['transport','Dovoz'],['payment-protection','Sigurno plaćanje'],['base','Auto plac Čalije'],['wissen','Vodiči'],['service-request','Pošalji upit']]
  ...
  const subtitle=sr?'PAMETAN UVOZ AUTOMOBILA · NEMAČKA / ŠVAJCARSKA → SRBIJA':'FAHRZEUGIMPORT MIT PRÜFUNG UND KOSTENKONTROLLE';
  const body=
  ...
  '<main class="wrap">'+
  '<section class="hero"><div><p class="eyebrow">'+(sr?'SVE PRE NEGO ŠTO DAŠ NOVAC ZA AUTO':'ALLES PRÜFEN, BEVOR DU ZAHLST')+'</p><h1>'+(sr?'Želiš auto iz Nemačke ili Švajcarske? Prvo proveri šta te stvarno čeka.':'Auto aus Deutschland oder der Schweiz? Erst prüfen, dann kaufen.')+'</h1><p class="lead">'+(sr?'Na jednom mestu vidiš okvirnu ukupnu cenu do Srbije, rizik porekla i dokumenata, poznate slabosti modela i opcije za pregled, delove i dovoz. Bez mešanja nemačkog, engleskog i stručnih izraza koje niko ne razume.':'Auf einer Seite siehst du Gesamtkosten, Herkunfts- und Dokumentenrisiko, bekannte Modellschwächen sowie Optionen für Prüfung, Teile und Überführung.')+'</p><div class="hero-actions"><a class="btn" href="#passport">'+(sr?'Proveri konkretan auto':'Konkretes Auto prüfen')+'</a><a class="btn secondary" href="#service-request">'+(sr?'Treba mi usluga':'Service anfragen')+'</a></div><div class="trust"><span>'+(sr?'Besplatna početna provera':'Kostenloser Erstcheck')+'</span><span>'+(sr?'Kompletan izveštaj 9,90 €':'Vollreport 9,90 €')+'</span><span>'+(sr?'Pregled vozila u Nemačkoj':'Fahrzeugcheck in Deutschland')+'</span></div></div><div class="hero-visual"><img src="/visual-uvoz.svg" alt="'+(sr?'Uvoz automobila iz Nemačke i Švajcarske u Srbiju':'Fahrzeugimport Deutschland und Schweiz nach Serbien')+'"></div></section>'+
  ...
  '<section class="section"><p class="eyebrow">'+(sr?'USLUGE I CENE':'LEISTUNGEN UND PREISE')+'</p><h2>'+(sr?'Ovde se vidi šta može da se naruči i kako DANINI zarađuje.':'Hier ist klar, was bestellt werden kann und wofür bezahlt wird.')+'</h2><div class="money-grid">'+
  '<article class="card"><h3>'+(sr?'Kompletan izveštaj o autu':'Vollständiger Fahrzeugreport')+'</h3><div class="price">9,90 €</div><p>'+(sr?'Trošak, poreklo, rizici, istorija modela i koraci pre kupovine.':'Kosten, Ursprung, Risiken, Modellhistorie und nächste Schritte.')+'</p></article>'+
  '<article class="card"><h3>'+(sr?'Obilazak vozila uživo':'Live-Fahrzeugbesichtigung')+'</h3><div class="price">'+(sr?'od 79 €':'ab 79 €')+'</div><p>'+(sr?'Cena zavisi od udaljenosti od Duisburga.':'Preis abhängig von der Entfernung ab Duisburg.')+'</p></article>'+
  ...
  '<article class="card"><h3>'+(sr?'Originalni delovi':'Originalteile')+'</h3><div class="price"><small>'+(sr?'ponuda po VIN-u / delu':'Angebot nach VIN / Teil')+'</small></div><p>'+(sr?'Cena dela i dostave prikazuje se odvojeno.':'Teile- und Versandpreis werden getrennt ausgewiesen.')+'</p></article>'+
  '<article class="card"><h3>'+(sr?'Auto plac Čalije':'Import-Basis Čalije')+'</h3><div class="price"><small>'+(sr?'zakup na upit':'Miete auf Anfrage')+'</small></div><p>'+(sr?'Blok A, Blok B ili deo površine, nakon pravne i tehničke potvrde.':'Block A, Block B oder Teilfläche nach rechtlicher und technischer Prüfung.')+'</p></article>'+
  '</div></section>'+
  ...
  '<section class="section" id="base"><p class="eyebrow">'+(sr?'AUTO PLAC U NIŠU':'IMPORT-BASIS IN NIŠ')+'</p><h2>'+(sr?'Čalije: fizička baza za uvoznike.':'Čalije: physische Basis für Importeure.')+'</h2><p class="muted">'+(sr?'Blok A, Blok B ili deo površine za prodaju, privremeno držanje vozila, pregled kupaca i primopredaju. Kapacitet, prilaz, dozvoljena namena i zakup se potvrđuju pre ugovora.':'Block A, Block B oder Teilfläche für Verkauf, Zwischenabstellung, Besichtigung und Übergabe. Kapazität, Zufahrt, Nutzung und Miete werden vor Vertrag bestätigt.')+'</p><a class="btn" href="https://calije.daninihub.com/" target="_blank" rel="noreferrer">'+(sr?'Pogledaj Čalije lokaciju':'Čalije Standort ansehen')+'</a></section>'+
  ...
  '<section class="section" id="service-request"><p class="eyebrow">'+(sr?'POŠALJI KONKRETAN UPIT':'KONKRETE ANFRAGE SENDEN')+'</p><h2>'+(sr?'Koji auto, koja usluga, gde se nalazi?':'Welches Auto, welche Leistung, welcher Standort?')+'</h2><form class="form" id="service-form"><label>'+(sr?'Usluga':'Leistung')+'<select name="serviceType">'+serviceOptions(sr)+'</select></label><label>'+(sr?'Ime / firma':'Name / Firma')+'<input name="name" required></label><label>Email<input name="email" type="email" required></label><label>Telefon<input name="phone"></label><label>'+(sr?'Vozilo':'Fahrzeug')+'<input name="vehicle"></label><label>VIN<input name="vin"></label><label>'+(sr?'Broj dela / OEM':'Teilenummer / OEM')+'<input name="partNumber"></label><label>'+(sr?'Gde je auto':'Standort')+'<input name="pickupLocation"></label><label>'+(sr?'Gde treba da stigne':'Ziel')+'<input name="destination"></label><label class="wide">'+(sr?'Šta tačno treba':'Details')+'<textarea name="message"></textarea></label><label class="wide"><span><input name="privacyAcknowledged" type="checkbox" required> '+(sr?'Pročitao/la sam obaveštenje o privatnosti.':'Ich habe die Datenschutzhinweise gelesen.')+'</span></label><button class="btn wide" type="submit">'+(sr?'POŠALJI UPIT':'ANFRAGE SENDEN')+'</button></form><div id="service-result"></div></section>'+
...
function knowledgePage(route){
  ...
  const body='<header class="head"><div class="top"><a class="brand" href="/'+meta.lang+'/"><img src="/importos-mark.svg" alt="DANINI"><div><strong>DANINI</strong><small>'+(sr?'PAMETAN UVOZ AUTOMOBILA':'FAHRZEUGIMPORT')+'</small></div></a></div></header><main class="wrap"><section class="section"><p class="eyebrow">'+(sr?'PRAKTIČNI VODIČ':'PRAXIS-RATGEBER')+'</p><h2>'+esc(title)+'</h2><p class="lead">'+esc(desc)+'</p><div class="grid"><article class="card"><h3>'+(sr?'Zašto je važno':'Warum wichtig')+'</h3><p>'+(sr?'Kupovina se donosi tek kada su troškovi, dokumenti i rizik dovoljno jasni.':'Kaufen erst, wenn Kosten, Dokumente und Risiko ausreichend klar sind.')+'</p></article><article class="card"><h3>'+(sr?'Pravilo':'Regel')+'</h3><p>'+(sr?'Ne računamo najpovoljniji scenario kao siguran dok dokaz nije potvrđen.':'Das günstigste Szenario gilt erst nach belegbarer Prüfung.')+'</p></article><article class="card"><h3>'+(sr?'Sledeći korak':'Nächster Schritt')+'</h3><p>'+(sr?'Vrati se na početnu stranu, pokreni proveru ili pošalji konkretan upit za vozilo.':'Zur Startseite, Fahrzeug prüfen oder konkrete Anfrage senden.')+'</p></article></div></section></main>';
  return shell({lang:meta.lang,title:meta.title,description:meta.description,body});
}
