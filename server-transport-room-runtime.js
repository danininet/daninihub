...
function initialTransportRoomCase(caseId = 'DH-TR-0001') {
  const id = clean(caseId, 64).toUpperCase();
  if (id === 'DH-TR-0002') {
    return {
      caseId: id, fictitious: true, route: 'Dortmund → Zagreb', partner: 'Danube Logistics Demo d.o.o.',
      vehicle: 'ZG-TEST-202', driver: 'DEMO DRIVER 2', owner: 'RheinCargo Demo GmbH', status: 'IN_TRANSIT', eta: '21:10', nextCheck: '18:30', risk: 'MEDIUM',
      approvedMessage: false,
      standardizedMessage: 'Vehicle is in transit from Dortmund to Zagreb. Current ETA is operational and must be reconfirmed at the next control point.',
      documents: { order: 'PRESENT', cmr: 'PRESENT', pod: 'MISSING', insurance: 'REVIEW' },
      incident: { status: 'MONITORING', severity: 'MEDIUM', decision: 'Confirm border waiting time and update the unloading contact if ETA changes.' },
      timeline: [
        { at: '09:15', status: 'ORDER_RECORDED', note: 'Transport order recorded for Dortmund to Zagreb.' },
        { at: '13:40', status: 'IN_TRANSIT', note: 'Vehicle departed the loading location.' },
        { at: '17:20', status: 'IN_TRANSIT', note: 'Operational ETA set to 21:10; next verification at 18:30.' }
      ],
      updatedAt: new Date().toISOString()
    };
  }
  if (id !== 'DH-TR-0001') return null;
  return {
    caseId: id, fictitious: true, route: 'Duisburg → Beograd', partner: 'Danube Logistics d.o.o.',
...
function validatePayload(body) {
  ...
  const timeline = Array.isArray(body.timeline)
    ? body.timeline.slice(-50).map(item => ({ at: clean(item.at, 20), status: clean(item.status, 40).toUpperCase(), note: clean(item.note, 500), actor: clean(item.actor, 180), role: clean(item.role, 40) })).filter(item => allowedStatuses.has(item.status) && item.note)
    : [];
  return { value: {
    caseId, fictitious: true, route: clean(body.route,160), partner: clean(body.partner,160), vehicle: clean(body.vehicle,80), driver: clean(body.driver,120), owner: clean(body.owner,120), status,
    eta: clean(body.eta,40), nextCheck: clean(body.nextCheck,40), risk: clean(body.risk,20).toUpperCase(), approvedMessage: body.approvedMessage === true,
    standardizedMessage: clean(body.standardizedMessage,2000), documents: Object.fromEntries(Object.entries(documents).map(([k,v])=>[k,clean(v,20).toUpperCase()])),
...
function enforceRole(existing, proposed, role) {
  ...
  for (const field of ['route','partner','vehicle','driver','owner','status','eta','nextCheck','risk','approvedMessage','standardizedMessage','documents','incident','timeline']) {
...
function mountTransportRoomRuntime(app, options = {}) {
  ...
  app.post('/api/v1/transport-room/access', async (req, res) => {
    const caseId = clean(req.body?.caseId,64).toUpperCase();
    const role = clean(req.body?.role,40).toUpperCase();
    if (!allowedRoles.has(role)) return res.status(400).json({ ok:false,error:'INVALID_DEMO_ACCESS_REQUEST' });
    const record = await store.get(caseId);
    const seed = initialTransportRoomCase(caseId);
    if (!record?.payload && !seed) return res.status(404).json({ ok:false,error:'TRANSPORT_CASE_NOT_FOUND' });
    if (!record?.payload && seed) await store.upsert({ caseId, status:seed.status, approval:'PENDING', payload:seed });
    const expiresAt = Date.now() + 2*60*60*1000;
    return res.json({ ok:true,caseId,role,identity:'demo-user',expiresAt,token:signAccess({caseId,role,identity:'demo-user',kind:'DEMO',exp:expiresAt}) });
  });
  ...
  app.post('/api/v1/transport-room/:caseId/invitations', async (req,res) => {
    ...
    let delivery;
    try { delivery = await sendEmail(email,`DaniniHub Transport Room – Einladung ${caseId}`,`<h2>DaniniHub Transport Room</h2><p>Sie wurden für <strong>${caseId}</strong> eingeladen.</p><p><a href="${invite.link}">Transport Room öffnen</a></p><p>Nach dem Öffnen erhalten Sie einen einmaligen Sicherheitscode per E-Mail.</p>`); }
  ...
  app.post('/api/v1/transport-room/invitations/start', async (req,res) => {
    ...
    let decoded;
    try { decoded=JSON.parse(Buffer.from(token.split('.')[0],'base64url').toString('utf8')); }
    ...
    const record = await inviteStore.get(access.inviteId);
    const invite = record?.payload;
    ...
    const code = newCode();
    const challengeId=`OTP-${crypto.randomBytes(8).toString('hex')}`;
    ...
    let delivery;
    try { delivery=await sendEmail(access.identity,`DaniniHub Sicherheitscode ${access.caseId}`,`<h2>Ihr Sicherheitscode</h2><p style="font-size:28px;font-weight:700;letter-spacing:6px">${code}</p><p>Der Code ist 10 Minuten gültig.</p>`); }
    catch(error){delivery={sent:false,reason:error.message};}
  ...
  app.post('/api/v1/transport-room/invitations/verify', async (req,res) => {
    ...
    const record=await otpStore.get(challengeId);
    const otp=record?.payload;
    ...
    if (hashCode(code)!==otp.codeHash) {
      otp.attempts=Number(otp.attempts)+1;
      await otpStore.upsert({caseId:challengeId,status:'PENDING',approval:otp.inviteId,payload:otp});
      return res.status(401).json({ok:false,error:'OTP_INCORRECT',attemptsRemaining:Math.max(0,5-otp.attempts)});
    }
    const inviteRecord=await inviteStore.get(otp.inviteId);
    const invite=inviteRecord?.payload;
    ...
    const expiresAt=Math.min(Number(invite.expiresAt),Date.now()+8*60*60*1000);
    const sessionToken=signAccess({caseId:invite.caseId,role:invite.role,identity:invite.email,inviteId:invite.inviteId,kind:'SESSION',exp:expiresAt});
  ...
  app.get('/api/v1/transport-room/:caseId', async (req,res) => {
    ...
    let record=await store.get(caseId);
    let payload=record?.payload;
    if (!payload) {
      payload=initialTransportRoomCase(caseId);
      if (payload) record=await store.upsert({caseId,status:payload.status,approval:'PENDING',payload});
    }
  ...
  app.put('/api/v1/transport-room/:caseId', async (req,res) => {
    ...
    const existing=existingRecord?.payload||initialTransportRoomCase(caseId);
    if (!existing) return res.status(404).json({ok:false,error:'TRANSPORT_CASE_NOT_FOUND'});
